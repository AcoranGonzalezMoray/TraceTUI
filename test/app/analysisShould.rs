#[cfg(test)]
mod analysis_tests {
    use crate::app::services::analysis_service;
    use crate::app::App;

    #[test]
    fn test_on_tick_increments_frame_count() {
        let mut app = App::new();
        let before = app.ui.frame_count;
        analysis_service::on_tick(&mut app);
        assert_eq!(app.ui.frame_count, before.wrapping_add(1));
    }
    ...
    fn test_on_tick_multiple() {
        ...
        assert_eq!(app.ui.frame_count, before.wrapping_add(10));
    }
    ...
    fn test_background_refresh_skips_when_rx_active() {
        ...
        app.network.data_rx = Some(rx);
        analysis_service::trigger_background_refresh(&mut app);
        assert!(app.network.data_rx.is_some());
        drop(tx);
    }
    ...
    fn test_trigger_geo_lookup_no_selection() {
        ...
        assert_eq!(app.geo.pending_geo_lookups, before);
    }
    ...
    fn test_continuous_refresh_counter_wraps() {
        let mut app = App::new();
        app.ui.continuous_refresh_counter = u64::MAX;
    ...
    fn test_analysis_not_complete_blocks_refresh() {
        let mut app = App::new();
        app.ui.auto_analysis_complete = false;
