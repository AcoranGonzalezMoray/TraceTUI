...
pub fn is_newer(local: &str, remote: &str) -> bool {
    ...
    pub fn on_tick(app: &mut App) {
        ...
        if app.ui.auto_analysis_complete
            ...
            && app.storage.last_storage_refresh.elapsed() >= std::time::Duration::from_secs(config::STORAGE_REFRESH_INTERVAL_SECS)
        {
            ...
            if !app.storage.file_search_mode && !app.storage.search_progress_running {
                ...
                app.storage.file_entries = crate::app::storage::StorageManager::list_directory(&current)
                    .unwrap_or_default();
    ...
    fn refresh_processes(app: &mut App) {
        ...
        if manager.refresh_processes().is_ok() {
            ...
            app.ui.status_message = tr!(app.ui.translator, "status.found_procs", app.network.processes.len());
        } else {
    ...
    pub fn start_investigation(app: &mut App) {
        if let Some(app_conn) = app.get_selected_app() {
            if let Some(conn) = app_conn.connections.get(app.network.selected_connection_index) {
