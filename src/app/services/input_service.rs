...
use crossterm::event::{
    ...
    fn handle_dashboard_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            ...
            KeyCode::Up => {
                let in_investigation = app.investigation.investigation_report.is_some() || app.investigation.is_investigating;
            ...
            KeyCode::Char('q') | KeyCode::Char('Q') | KeyCode::Esc => {
                if app.investigation.investigation_report.is_some() || app.investigation.is_investigating {
    ...
    fn handle_container_keys(app: &mut App, key: KeyEvent) {
        ...
        match key.code {
            ...
            KeyCode::Char('n') | KeyCode::Char('N') => {
                app.containers.selected_container_action_index = crate::app::containers::DOCKER_ACTION_OFFSET;
                app.execute_container_right_action();
            }
    ...
    fn handle_container_logs_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            ...
            KeyCode::Up => {
                app.containers.container_logs_scroll = app.containers.container_logs_scroll.saturating_sub(1);
            }
            KeyCode::Down => {
                let max = app.containers.container_logs.len().saturating_sub(1);
                app.containers.container_logs_scroll = app.containers.container_logs_scroll.saturating_add(1).min(max);
            }
            KeyCode::PageUp => {
                app.containers.container_logs_scroll = app.containers.container_logs_scroll.saturating_sub(10);
            }
            KeyCode::PageDown => {
                let max = app.containers.container_logs.len().saturating_sub(1);
                app.containers.container_logs_scroll = app.containers.container_logs_scroll.saturating_add(10).min(max);
            }
            KeyCode::End => {
                app.containers.container_logs_scroll = app.containers.container_logs.len().saturating_sub(1);
            }
    ...
    fn handle_container_console_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            ...
            KeyCode::Up => {
                app.containers.container_console_scroll = app.containers.container_console_scroll.saturating_sub(1);
            }
            KeyCode::Down => {
                let max = app.containers.container_console_output.len().saturating_sub(1);
                app.containers.container_console_scroll =
                    app.containers.container_console_scroll.saturating_add(1).min(max);
            }
    ...
    fn handle_docker_hub_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            KeyCode::Esc => {
                ...
                app.containers.docker_hub_search = crate::app::containers::DockerHubSearchState::default();
            }
            KeyCode::Tab => {
                app.containers.docker_hub_search.focused_field =
                    (app.containers.docker_hub_search.focused_field + 1) % config::DOCKER_HUB_FIELD_COUNT;
            }
            KeyCode::BackTab => {
                app.containers.docker_hub_search.focused_field = if app.containers.docker_hub_search.focused_field == 0
                {
            ...
            KeyCode::Up | KeyCode::Down if app.containers.docker_hub_search.focused_field == 0 => {
                if !app.containers.docker_hub_search.results.is_empty() {
                    match key.code {
                        ...
                        KeyCode::Down => {
                            let max = app.containers.docker_hub_search.results.len().saturating_sub(1);
    ...
    fn start_docker_hub_search_async(app: &mut App) {
        if app.containers.docker_hub_search.search_query.is_empty() {
            app.ui.status_message =
                tr!(app.ui.translator, "containers.docker_hub_search_placeholder").to_string();
            return;
        }
    ...
    fn handle_library_binary_viewer_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            KeyCode::Up | KeyCode::Char('k') => {
                app.libraries.library_binary_scroll = app.libraries.library_binary_scroll.saturating_sub(1);
            }
            ...
            KeyCode::PageUp => {
                app.libraries.library_binary_scroll = app.libraries.library_binary_scroll.saturating_sub(20);
            }
            KeyCode::PageDown => {
                let total = if app.libraries.library_binary_tab == 0 {
                    app.libraries.library_binary_hex_lines.len()
            ...
            KeyCode::BackTab => {
                app.libraries.library_binary_tab = app.libraries.library_binary_tab.saturating_sub(1) % 2;
                app.libraries.library_binary_scroll = 0;
            }
            KeyCode::Esc => {
        ...
        match key.code {
            ...
            KeyCode::Up if app.ui.sidebar_focus == SidebarFocus::Left => {
                if app.libraries.selected_library_process_index > 0 {
                    ...
                    app.libraries.selected_library_index = 0;
                    app.libraries.library_lib_scroll = 0;
                    app.libraries.library_process_scroll = app.libraries.library_process_scroll.saturating_sub(0);
                }
            ...
            KeyCode::PageUp if app.ui.sidebar_focus == SidebarFocus::Left => {
                app.libraries.selected_library_process_index =
                    app.libraries.selected_library_process_index.saturating_sub(10);
                app.libraries.selected_library_index = 0;
                app.libraries.library_lib_scroll = 0;
            }
            ...
            KeyCode::PageUp if app.ui.sidebar_focus == SidebarFocus::Center => {
                app.libraries.selected_library_index = app.libraries.selected_library_index.saturating_sub(10);
            ...
            KeyCode::PageDown if app.ui.sidebar_focus == SidebarFocus::Center => {
                let libs = crate::app::ui::libraries::get_libs_for_selected_process(app);
                let max = libs.len().saturating_sub(1);
                app.libraries.selected_library_index = (app.libraries.selected_library_index + 10).min(max);
            }
            ...
            KeyCode::Esc | KeyCode::Char('q') | KeyCode::Char('Q') => {
                ...
                } else if app.investigation.investigation_report.is_some() || app.investigation.is_investigating {
                    app.investigation.investigation_report = None;
                    app.investigation.is_investigating = false;
                    app.ui.analysis_paused = false;
                } else {
    ...
    fn handle_file_viewer_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            ...
            KeyCode::Down => {
                ...
                app.storage.file_viewer_scroll = app.storage.file_viewer_scroll.saturating_add(1).min(max);
            }
            KeyCode::PageUp => {
            ...
            KeyCode::PageDown => {
                let max = app.storage.file_viewer_content.len().saturating_sub(1);
                app.storage.file_viewer_scroll = app.storage.file_viewer_scroll.saturating_add(20).min(max);
            }
            KeyCode::End => {
                app.storage.file_viewer_scroll = app.storage.file_viewer_content.len().saturating_sub(1);
            }
            _ => {}
        }
    }
    ...
    fn handle_install_dialog_keys(app: &mut App, key: KeyEvent) {
        ...
        match key.code {
            KeyCode::Char('y') | KeyCode::Char('Y') | KeyCode::Enter => {
                ...
                app.install.message =
                    tr!(app.ui.translator, "dialog.net_tools_checking").to_string();
    ...
    fn handle_password_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            KeyCode::Esc => {
                ...
                app.install.message =
                    tr!(app.ui.translator, "dialog.password_cancelled").to_string();
                app.ui.status_message = tr!(app.ui.translator, "status.install_cancelled").to_string();
            }
    ...
    fn handle_firewall_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            ...
            KeyCode::Tab => {
                app.firewall.firewall_focus = FirewallManager::cycle_focus_forward(app.firewall.firewall_focus);
                app.firewall.firewall_action_index = 0;
            }
            KeyCode::BackTab => {
                app.firewall.firewall_focus = FirewallManager::cycle_focus_backward(app.firewall.firewall_focus);
                app.firewall.firewall_action_index = 0;
            }
            ...
            KeyCode::Char(' ') => match app.firewall.firewall_focus {
                FirewallPanel::Connections => {
                    if let Some(checked) =
                        app.firewall.firewall_conn_checked.get_mut(app.firewall.firewall_conn_index)
                    {
                        *checked = !*checked;
                    }
                }
    ...
    fn firewall_scroll(app: &mut App, delta: i32) {
        match app.firewall.firewall_focus {
            FirewallPanel::Connections => {
                ...
                app.firewall.firewall_conn_index = apply_scroll(app.firewall.firewall_conn_index, delta, max);
            }
            FirewallPanel::BlockedList => {
                let max = app.firewall.blocked_ips.len().saturating_sub(1);
                app.firewall.firewall_blocked_index = apply_scroll(app.firewall.firewall_blocked_index, delta, max);
            }
            FirewallPanel::Actions => {
                let max = FirewallManager::get_firewall_action_count();
                app.firewall.firewall_action_index = apply_scroll(app.firewall.firewall_action_index, delta, max);
            }
        }
    }
            ...
            match mouse.kind {
                MouseEventKind::ScrollDown => {
                    ...
                    app.containers.container_logs_scroll =
                        app.containers.container_logs_scroll.saturating_add(1).min(max);
                }
    ...
    fn handle_mouse_scroll(app: &mut App, delta: i32) {
        if app.ui.current_nav_view == NavView::LibraryInspection && app.libraries.show_library_binary_viewer {
            let max = if app.libraries.library_binary_tab == 0 {
                app.libraries.library_binary_hex_lines.len().saturating_sub(1)
            } else {
                app.libraries.library_binary_disasm_lines.len().saturating_sub(1)
            };
            app.libraries.library_binary_scroll = apply_scroll(app.libraries.library_binary_scroll, delta, max);
            return;
        }
        if app.ui.current_nav_view == NavView::Storage && app.storage.show_file_viewer {
            let max = app.storage.file_viewer_content.len().saturating_sub(1);
        ...
        match app.ui.sidebar_focus {
            ...
            SidebarFocus::Left => {
                if app.ui.current_nav_view == NavView::Containers {
                    ...
                    let max = app.storage.disks.len().saturating_sub(1);
                    app.storage.selected_disk_index = apply_scroll(app.storage.selected_disk_index, delta, max);
                ...
                } else if app.investigation.investigation_report.is_none() && !app.investigation.is_investigating {
                    let max = app.get_filtered_apps().len().saturating_sub(1);
                    if apply_scroll_bool(app.network.selected_app_index, delta, max) {
                        app.network.selected_app_index = apply_scroll(app.network.selected_app_index, delta, max);
                        app.trigger_geo_lookup_for_selected_app();
                    }
                }
            }
        ...
        match app.ui.selected_action_index {
            ...
            6 => {
                ...
                if let Some(lib) = libs.get(app.libraries.selected_library_index) {
                    ...
                    app.libraries.library_binary_hex_lines = crate::app::libraries::load_binary_hex(&path);
        ...
        match app.firewall.firewall_action_index {
            ...
            1 => {
                let to_block: Vec<String> = app
                    ...
                    .iter()
                    .enumerate()
                    .filter(|(i, _)| app.firewall.firewall_conn_checked.get(*i).copied().unwrap_or(false))
            ...
            2 => {
                let to_unblock: Vec<String> = app
                    ...
                    .enumerate()
                    .filter(|(i, _)| {
                        app.firewall.firewall_blocked_checked
                            .get(*i)
                            .copied()
                            .unwrap_or(false)
                    })
    ...
    pub fn toggle_selected_conn_checkbox(app: &mut App) {
        if let Some(checked) = app.firewall.firewall_conn_checked.get_mut(app.firewall.firewall_conn_index) {
            *checked = !*checked;
        }
    }
    ...
    fn handle_welcome_keys(app: &mut App, key: KeyEvent) {
        match key.code {
            KeyCode::Left | KeyCode::Right => {
                app.ui.welcome_index =
                    (app.ui.welcome_index + 1) % crate::config::WELCOME_PAGE_COUNT;
            }
            KeyCode::Enter => {
