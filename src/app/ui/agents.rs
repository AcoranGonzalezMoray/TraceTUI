    ...
    while byte_idx < text.len() {
        ...
        if remaining.starts_with("**") {
            ...
            if let Some(pos) = end {
                ...
                spans.push(Span::styled("**".to_string(), Style::default().fg(THEME.text_main)));
                byte_idx += 2;
            }
...
fn filtered_markdown_lines(app: &App, text: &str, width: usize) -> Vec<Line<'static>> {
    ...
    let mut lines = markdown_to_lines(&source.join("\n"), width);
    if app.agents.agent_search_mode || !app.agents.agent_search_query.is_empty() {
        lines.insert(
            0,
            Line::from(Span::styled(
                format!("/{}", app.agents.agent_search_query),
...
fn render_agents_center(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            ...
            Style::default().fg(status_color(&agent.status)).add_modifier(Modifier::BOLD),
        ),
    ]));
    let mut button_spans = Vec::new();
    for span in action_button("⏹", tr!(app.ui.translator, "agents.action_cancel"), "S", THEME.danger) {
        button_spans.push(span);
    }
    for span in action_button("󰒈", tr!(app.ui.translator, "agents.action_export_md"), "E", THEME.secondary) {
        button_spans.push(span);
    }
    for span in action_button("󰘦", tr!(app.ui.translator, "agents.action_export_json"), "J", THEME.secondary) {
        button_spans.push(span);
    }
    for span in action_button("󰍉", tr!(app.ui.translator, "agents.action_filter"), "/", THEME.primary) {
        button_spans.push(span);
    }
    for span in action_button(
        if app.agents.collapse_sections { "󰅀" } else { "󰅂" },
        if app.agents.collapse_sections {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(format!("{} / {}", agent.provider.label(), agent.model), Style::default().fg(THEME.text_main)),
    ]));
    ...
    match &agent.status {
        ...
        AgentStatus::Running(msg) => {
            ...
            let progress = ((elapsed as usize * 2).min(95)).max(if msg.is_empty() { 5 } else { 20 });
            let eta_secs = 180usize.saturating_sub(elapsed as usize / 4);
            let filled = progress / 10;
            let bar = format!("[{}{}] {}% ~{}s", "█".repeat(filled), "░".repeat(10usize.saturating_sub(filled)), progress, eta_secs);
...
fn render_agents_right(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    let action_items: Vec<(&str, String, &str, ratatui::style::Color)> = vec![
        (
            "󰈐",
            format!("{} ({})", tr!(t, "agents.action_provider"), app.agents.ollama.provider.label()),
            "C",
            THEME.secondary,
        ),
        (
            "󰚩",
            tr!(t, "agents.action_launch"),
            "Enter",
            THEME.primary,
        ),
        (
            "󰑐",
            tr!(t, "agents.action_retry"),
            "R",
            THEME.warning,
        ),
        (
            "󰩈",
            tr!(t, "agents.action_clear"),
            "X",
            THEME.danger,
        ),
    ];
    ...
    f.render_stateful_widget(List::new(items).block(Block::default()), list_area, &mut list_state);
...
pub fn render_provider_modal(f: &mut ratatui::Frame, app: &App) {
    ...
    lines.push(Line::from(vec![
        Span::styled(
            "  Provider: ",
            if has_focus == 0 { Style::default().fg(THEME.primary).add_modifier(Modifier::BOLD) } else { Style::default().fg(THEME.text_dim) },
        ),
        Span::styled(app.agents.ollama.provider.label(), Style::default().fg(THEME.text_main)),
        Span::styled(
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            ...
            Style::default()
                .fg(THEME.background)
                .bg(if has_focus == 5 { THEME.primary } else { THEME.secondary })
                .add_modifier(Modifier::BOLD),
        ),
        Span::raw("  "),
        Span::styled(
            ...
            Style::default()
                .fg(THEME.background)
                .bg(if has_focus == 6 { THEME.primary } else { THEME.success })
                .add_modifier(Modifier::BOLD),
        ),
        Span::raw("  "),
        Span::styled(
            ...
            Style::default()
                .fg(THEME.background)
                .bg(if has_focus == 7 { THEME.primary } else { THEME.danger })
                .add_modifier(Modifier::BOLD),
        ),
    ]));
...
pub fn render_agent_type_selector(f: &mut ratatui::Frame, app: &App) {
    ...
    let items = [
        ("󰆧", tr!(app.ui.translator, "agents.launch_process"), tr!(app.ui.translator, "agents.agent_process_desc")),
        ("󰛳", tr!(app.ui.translator, "agents.launch_network"), tr!(app.ui.translator, "agents.agent_network_desc")),
        ("󰖟", tr!(app.ui.translator, "agents.agent_dns"), tr!(app.ui.translator, "agents.agent_dns_desc")),
        ("󰈙", tr!(app.ui.translator, "agents.agent_file"), tr!(app.ui.translator, "agents.agent_file_desc")),
        ("󰓾", tr!(app.ui.translator, "agents.agent_ports"), tr!(app.ui.translator, "agents.agent_ports_desc")),
        ("󰌱", tr!(app.ui.translator, "agents.agent_logs"), tr!(app.ui.translator, "agents.agent_logs_desc")),
        ("󰍛", tr!(app.ui.translator, "agents.agent_memory"), tr!(app.ui.translator, "agents.agent_memory_desc")),
        ("󰒃", tr!(app.ui.translator, "agents.agent_vuln"), tr!(app.ui.translator, "agents.agent_vuln_desc")),
        ("󰳦", tr!(app.ui.translator, "agents.agent_intel"), tr!(app.ui.translator, "agents.agent_intel_desc")),
    ];
